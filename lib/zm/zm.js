...
function loadJednaniPage (opts, DB, url) {
  ...
  .then($ => {
    ...
    const datum = name.match(/\d{2}.\d{2}.\d{4}/)[0]
    ...
    return DB.saveJednaniZM({
      name, link: url, datum, usneseni, hlasovani
    })
  })
  .then(jednani => {
    let items = null
    ...
    .then(doc => {
      const promises = items.map(i => {
        const bod = Object.assign(i, {jednani_fk: jednani.id})
        return DB.saveBodJednaniZM(bod)
        .catch(err => console.log(err.detail + JSON.stringify(bod, null, 4)))
      })
